...
class CPI:
    ...
    name = "CPI - Coinitial Propagation of Independence"
                ...
                while len(edges)>0:
                    ...
                    for edge2 in edges:
                        ...
                        for end in graph.nodes:
                            ...
                            if graph.EdgeExists(first) and graph.EdgeExists(second) and (graph.AreIndependent(edge1, edge2)  or (edge1, edge2) in independence_to_add):
                                
                                #if there is not u' ι nt => we save those edges as errors
                                rev1 = ReverseEdge(edge1)
                                if not (graph.AreIndependent(rev1, first) or (rev1,first) in independence_to_add):
                                    independence_to_add.append((rev1, first))
                                    independence_to_add.append((first, rev1))
                                    errors.add((edge1, edge2, rev1, first))
                                    keep_search = True

                                rev2 = ReverseEdge(edge2)
                                if not (graph.AreIndependent(rev2, second) or (rev2,second) in independence_to_add):
                                    independence_to_add.append((rev2, second))
                                    independence_to_add.append((second, rev2))
